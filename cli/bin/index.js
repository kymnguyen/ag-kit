...
const REPO = "github:kymnguyen/ag-kit";
...
const checkUpdate = async (quiet = false) => {
    ...
    try {
        ...
        const response = await fetch("https://registry.npmjs.org/@kymnguyen/ag-kit/latest", {
...
const showUpdateNotification = (latestVersion) => {
  ...
  │  Run: ${chalk.cyan("npm install -g @kymnguyen/ag-kit")}
  └────────────────────────────────────────────────────────┘
  `),
    );
};
